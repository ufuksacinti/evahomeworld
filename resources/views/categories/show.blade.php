...
<!-- Hero Slider Section -->
<section class="relative h-96 flex items-center justify-center overflow-hidden bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900">
    <!-- Background Image -->
    @if($category->image)
    ...
    <!-- Decorative Elements -->
    <div class="absolute top-10 left-10 w-32 h-32 rounded-full opacity-10 blur-3xl bg-primary"></div>
    <div class="absolute bottom-10 right-10 w-40 h-40 rounded-full opacity-10 blur-3xl bg-secondary"></div>
    
    <!-- Content -->
    <div class="relative z-10 container text-center text-white px-6 md:px-8 lg:px-10">
        <h1 class="text-5xl md:text-7xl font-bold mb-6 animate-fade-in">{{ $category->name }}</h1>
        <p class="text-xl md:text-2xl max-w-3xl mx-auto text-white/90 animate-fade-in-delay">
    ...
    <div class="container">
        ...
        <div class="section-header mb-8">
            <h2 class="text-3xl font-bold text-gray-800 mb-2">
                {{ $category->name }} Kategorisinden Ürünler
            </h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-4">
            ...
            <div class="product-card bg-white rounded-2xl shadow-lg overflow-hidden hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group flex flex-col">
                ...
                <div class="relative aspect-square bg-gray-100 overflow-hidden flex-shrink-0">
                    ...
                    @else
                        <div class="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/10 via-secondary/10 to-accent/10">
                    ...
                    @if($product->energyCollection)
                        <div class="absolute top-4 left-4 w-8 h-8 rounded-full shadow-lg border-2 border-white" 
                             style="background-color: {{ $product->energyCollection->color_code }};"></div>
                    @endif
                ...
                <div class="p-4 flex flex-col flex-grow">
                    <!-- Category Badge -->
                    <span class="inline-block text-xs font-semibold text-gray-500 mb-1 uppercase tracking-wide">
                        {{ $category->name }}
                    </span>
                    ...
                    <!-- Energy Collection Info -->
                    @if($product->energyCollection)
                    <div class="flex items-center mb-2">
                        <span class="w-3 h-3 rounded-full mr-2" style="background-color: {{ $product->energyCollection->color_code }};"></span>
                        <span class="text-xs text-gray-600">{{ $product->energyCollection->name }}</span>
                    </div>
                    @endif
        ...
        <div class="text-center py-16">
            <div class="inline-block mb-6 text-6xl">📦</div>
            <h3 class="text-2xl font-bold text-gray-800 mb-4">{{ $category->name }} Kategorisinde Henüz Ürün Yok</h3>
            <p class="text-gray-600 mb-8">Yakında bu kategoriden harika ürünler eklenecek!</p>
